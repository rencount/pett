<script setup lang="ts">
    ...
    function open(title:string,type:string,formData?:any){
      ...
      if (state.type == 'cross') {
        ...
        for (var o in obj) {
                                                                                                      if (o == 'biaoti') {
                    state.formData.biaoti = obj[o];
                    state.roisfalg.biaoti = true;
                    continue;
                  }
                                                              if (o == 'tupian') {
                    ...
                    state.formData.shangchuanshijian = obj[o];
                    state.roisfalg.shangchuanshijian = true;
                    continue;
                  }
                                                              if (o == 'clicktime') {
    ...
    function submitForm(){
        formRef.value?.validate((valid:boolean)=>{
          ...
          if (state.type == 'cross') {
            ...
            request({
              url:`xuexiguanli/${!state.formData.id ? "save" : "update"}`,
              method:'post',
              data:state.formData
            }).then(({data})=>{
              notify("操作成功",{type:'success'});
    ...
    <el-dialog :title="`${title}`" v-model="visible"
        ...
        <div v-loading="loadding">
            ...
            >
                        ...
                        <el-form-item label="标题" prop="biaoti" :rules="{required: true, message: '标题为必填项！', trigger: 'blur'}">
                            <el-input v-model="formData.biaoti" maxlength="30" clearable show-word-limit placeholder="请输入标题" :readonly="roisfalg.biaoti"/>
                        </el-form-item>
                            ...
                            <el-input v-model="formData.xiangqing" maxlength="100" clearable show-word-limit type="textarea" />
                        </el-form-item>


                                                                                <el-form-item label="上传时间" prop="shangchuanshijian"  :rules="{required: true, message: '上传时间为必填项', trigger: 'blur'}">

                            <el-date-picker
                                    v-model="formData.shangchuanshijian"
                                    type="date"
                                    format="YYYY/MM/DD"
