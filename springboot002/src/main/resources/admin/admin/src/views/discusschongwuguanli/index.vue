<script setup lang="ts">
  ...
  const AddEdit=defineAsyncComponent(()=>import('@/views/discusschongwuguanli/add-edit.vue'));
  ...
  function handleDelete(row:any){
    ...
    request({
      url:`discusschongwuguanli/delete`,
      method:'post',
      data:[row.id]
    }).then(({data})=>{
    ...
    <el-card shadow="hover">
      <div class="header-section">
        ...
        <div class="action-buttons">
            <el-button v-if="isAuth('discusschongwuguanli','新增')" icon="ele-Plus" type="primary" @click="add()">新增</el-button>
            <el-button v-if="isAuth('discusschongwuguanli','删除')" icon="ele-Delete" @click="listdel()" :disabled="isdel" type="danger" plain>批量删除</el-button>
                                                </div>
      </div>
      ...
      <el-table
        ...
        <el-table-column type="selection" width="55" align="center"/>
              ...
              <template #default="{row}">
                                                      <el-button v-if="isAuth('discusschongwuguanli','修改')" icon="ele-Edit" @click.stop="handleEdit(row)" type="primary" link>修改</el-button>
                  <el-popconfirm  v-if="isAuth('discusschongwuguanli','删除')"  width="auto" @confirm="handleDelete(row)"  :title="`确定要删除这条记录吗？`">
                      <template #reference>
