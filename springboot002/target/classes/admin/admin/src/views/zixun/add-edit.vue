<script setup lang="ts">
    ...
    function open(title:string,type:string,formData?:any){
      ...
      if (state.type == 'cross') {
        ...
        for (var o in obj) {
                                                                                                      if (o == 'chongwuyisheng') {
                    state.formData.chongwuyisheng = obj[o];
                    state.roisfalg.chongwuyisheng = true;
                    continue;
                  }
                                                              if (o == 'touxiang') {
    ...
    function submitForm(){
        formRef.value?.validate((valid:boolean)=>{
          ...
          if (state.type == 'cross') {
            ...
            request({
              url:`zixun/${!state.formData.id ? "save" : "update"}`,
              method:'post',
              data:state.formData
            }).then(({data})=>{
              notify("操作成功",{type:'success'});
    ...
    defineExpose({
        open
                    ...
                    request({
                        url: `option/chongwuyisheng/nicheng`,
                        method: "get"
                    }).then((data) => {
                        if (data && data.code === 0) {
                        state.chongwuyishengOptions = data.data;
                    } else {
...
<template>
    <el-dialog :title="`${title}`" v-model="visible"
        ...
        <div v-loading="loadding">
            ...
            >
                                                                                                                                    <el-form-item label="宠物医生" prop="chongwuyisheng" :rules="{required: true, message: '宠物医生为必选项！', trigger: 'blur'}">
                            <el-select
                                    v-model="formData.chongwuyisheng"
                                    clearable
                                    placeholder="请选择宠物医生"
                                    style="width: 240px"
                            >
                                <el-option
                                        v-for="(item,index) in chongwuyishengOptions"
                                        v-bind:key="index"
                                        :label="item"
                                        :value="item"
                                />
