        ...
        function getDataList(){
            ...
            const params={
                ...
                limit: state.page.size,
                                                                                                        ...
                                                                                                        if (state.searchForm.zhanghao != '' && state.searchForm.zhanghao != undefined) {
                        params['zhanghao'] = '%' + state.searchForm.zhanghao + '%'
                    }
...
<template ref="ragRefInstance">
  ...
  <div>
    ...
    <div class="py-20 bg-white">
      <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl font-bold text-center mb-4">宠物医生</h2>
        <div class="max-w-4xl mx-auto mb-12">
          <div class="flex items-center gap-4">

                                                                                                                                                                                                                                                      <div class="relative flex-1">
              <input type="text" placeholder="账号" v-model="searchForm.zhanghao" class="w-full px-4 py-2 border-2 border-gray-200 !rounded-button focus:outline-none focus:border-blue-500 pl-10 text-sm">
              <i class="fas fa-map-marker-alt absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
            </div>

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  <div class="relative flex-1">
              <input type="text" placeholder="昵称" v-model="searchForm.nicheng" class="w-full px-4 py-2 border-2 border-gray-200 !rounded-button focus:outline-none focus:border-blue-500 pl-10 text-sm">
              <i class="fas fa-map-marker-alt absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
            </div>
  ...
  <router-link :to="`/chongwuyishengdetail/`+item.id" class="shadow">
              <img :src="item.touxiang" class="w-full h-64 object-cover object-center transition-transform duration-300 group-hover:scale-110">
                                                                                            ...
                                                                                            <h3 class="mt-4 text-lg font-semibold">{{ item.zhanghao }}</h3>
                                                                            <h3 class="mt-4 text-lg font-semibold">{{ item.mima }}</h3>
                                                                                                                </div>
        </div>
      </div>
